...
void addNewColorAttach(
	FrameGraphBuilder::FrameGraphNode& node,
	const std::string& name,
	std::unordered_map<std::string, FrameGraphBuilder::FrameGraphResourceHandle>& resources)
{
...
TEST(VulkanTestSuite, FrameGraphBuildTest2)
{
	VulkanGlobal::init({}, {"VK_LAYER_KHRONOS_validation"}, false, nullptr, 0, 0, {}, {});
	FrameGraphBuilder builder;
	std::unordered_map<std::string, FrameGraphBuilder::FrameGraphResourceHandle> resources;
	{
		...
		colorAttachExtDep.viewOffset = 0;
		resources["attach_a0"] =
			a.addColorAttach(std::move(colorAttachmentRef), std::move(colorAttachment),
							 std::move(colorAttachExtDep));
	}
	{
		auto& b = builder.addSubpass();
