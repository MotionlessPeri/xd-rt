...
public:
	enum {
		Vectorizable =
			unpacket_traits<LhsPacket_>::vectorizable &&
...
struct general_matrix_vector_product<Index,
	...
	EIGEN_DEVICE_FUNC EIGEN_DONT_INLINE static void run(Index rows,
														Index cols,
														const LhsMapper& lhs,
														const RhsMapper& rhs,
														ResScalar* res,
											...
											RhsScalar alpha)
{
	...
	// The following copy tells the compiler that lhs's attributes are not modified outside this
	// function This helps GCC to generate propoer code.
	LhsMapper lhs(alhs);
	...
	for (Index j2 = 0; j2 < cols; j2 += block_cols) {
		...
		for (; i < n8; i += ResPacketSize * 8) {
			...
			for (Index j = j2; j < jend; j += 1) {
				...
				c0 = pcj.pmadd(
					lhs.template load<LhsPacket, LhsAlignment>(i + LhsPacketSize * 0, {}), b0, c0);
				c1 = pcj.pmadd(
					lhs.template load<LhsPacket, LhsAlignment>(i + LhsPacketSize * 1, {}), b0, c1);
				c2 = pcj.pmadd(
					lhs.template load<LhsPacket, LhsAlignment>(i + LhsPacketSize * 2, {}), b0, c2);
				c3 = pcj.pmadd(
					lhs.template load<LhsPacket, LhsAlignment>(i + LhsPacketSize * 3, {}), b0, c3);
				c4 = pcj.pmadd(
					lhs.template load<LhsPacket, LhsAlignment>(i + LhsPacketSize * 4, {}), b0, c4);
				c5 = pcj.pmadd(
					lhs.template load<LhsPacket, LhsAlignment>(i + LhsPacketSize * 5, {}), b0, c5);
				c6 = pcj.pmadd(
					lhs.template load<LhsPacket, LhsAlignment>(i + LhsPacketSize * 6, {}), b0, c6);
				c7 = pcj.pmadd(
					lhs.template load<LhsPacket, LhsAlignment>(i + LhsPacketSize * 7, {}), b0, c7);
			}
		...
		if (i < n1) {
			...
			for (Index j = j2; j < jend; j += 1) {
				RhsPacket b0 = pset1<RhsPacket>(rhs(j, 0));
				c0 = pcj.pmadd(lhs.template load<LhsPacket, LhsAlignment>(i + 0, {}), b0, c0);
			}
			pstoreu(res + i + ResPacketSize * 0,
					pmadd(c0, palpha, ploadu<ResPacket>(res + i + ResPacketSize * 0)));
			i += ResPacketSize;
		...
		if (HasHalf && i < n_half) {
			...
			for (Index j = j2; j < jend; j += 1) {
				...
				c0 = pcj_half.pmadd(lhs.template load<LhsPacketHalf, LhsAlignment>(i + 0, {}), b0,
									c0);
			}
		...
		if (HasQuarter && i < n_quarter) {
			...
			for (Index j = j2; j < jend; j += 1) {
				...
				c0 = pcj_quarter.pmadd(lhs.template load<LhsPacketQuarter, LhsAlignment>(i + 0, {}),
									   b0, c0);
			}
											...
											ResScalar alpha)
{
	...
	for (; i < n8; i += 8) {
		...
		for (Index j = 0; j < fullColBlockEnd; j += LhsPacketSize) {
			RhsPacket b0 = rhs.template load<RhsPacket, Unaligned>(j, {});

			c0 = pcj.pmadd(lhs.template load<LhsPacket, LhsAlignment>(i + 0, {}), b0, c0);
			c1 = pcj.pmadd(lhs.template load<LhsPacket, LhsAlignment>(i + 1, {}), b0, c1);
			c2 = pcj.pmadd(lhs.template load<LhsPacket, LhsAlignment>(i + 2, {}), b0, c2);
			c3 = pcj.pmadd(lhs.template load<LhsPacket, LhsAlignment>(i + 3, {}), b0, c3);
			c4 = pcj.pmadd(lhs.template load<LhsPacket, LhsAlignment>(i + 4, {}), b0, c4);
			c5 = pcj.pmadd(lhs.template load<LhsPacket, LhsAlignment>(i + 5, {}), b0, c5);
			c6 = pcj.pmadd(lhs.template load<LhsPacket, LhsAlignment>(i + 6, {}), b0, c6);
			c7 = pcj.pmadd(lhs.template load<LhsPacket, LhsAlignment>(i + 7, {}), b0, c7);
		}
	...
	for (; i < rows; ++i) {
		...
		for (Index j = 0; j < fullColBlockEnd; j += LhsPacketSize) {
			RhsPacket b0 = rhs.template load<RhsPacket, Unaligned>(j, {});
			c0 = pcj.pmadd(lhs.template load<LhsPacket, LhsAlignment>(i, {}), b0, c0);
		}
		ResScalar cc0 = predux(c0);
		if (HasHalf) {
			for (Index j = fullColBlockEnd; j < halfColBlockEnd; j += LhsPacketSizeHalf) {
				RhsPacketHalf b0 = rhs.template load<RhsPacketHalf, Unaligned>(j, {});
				c0_h =
					pcj_half.pmadd(lhs.template load<LhsPacketHalf, LhsAlignment>(i, {}), b0, c0_h);
			}
			cc0 += predux(c0_h);
		}
		if (HasQuarter) {
			for (Index j = halfColBlockEnd; j < quarterColBlockEnd; j += LhsPacketSizeQuarter) {
				RhsPacketQuarter b0 = rhs.template load<RhsPacketQuarter, Unaligned>(j, {});
				c0_q = pcj_quarter.pmadd(lhs.template load<LhsPacketQuarter, LhsAlignment>(i, {}),
										 b0, c0_q);
			}
			cc0 += predux(c0_q);
		}
