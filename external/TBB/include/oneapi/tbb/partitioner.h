...
class affinity_partitioner_base : no_copy {
	...
	//! Array that remembers affinities of tree rawPositions to affinity_id.
	...
	void resize(unsigned factor)
	{
		...
		if (new_size != my_size) {
			...
			if (new_size) {
				my_array =
					static_cast<slot_id*>(r1::cache_aligned_allocate(new_size * sizeof(slot_id)));
...
struct adaptive_mode : partition_type_base<Partition> {
	...
	// For affinity_partitioner, my_divisor indicates the number of affinity array indices the task
	// reserves. A task which has only one index must produce the right split without reserved index
	// in order to avoid it to be overwritten in note_affinity() of the created (right) task. I.e. a
	// task created deeper than the affinity array can remember must not save its affinity (LIFO
	// order)
	...
	std::size_t do_split(adaptive_mode& src, split) { return src.my_divisor /= 2u; }
};
...
struct proportional_mode : adaptive_mode<Partition> {
	...
	proportional_split get_split()
	{
		...
		//   otherwise, the tbb::proportional_split object will be implicitly (for Range
		//   implementer) casted to tbb::split
...
struct dynamic_grainsize_mode : Mode {
	...
	dynamic_grainsize_mode() : Mode(), my_delay(begin), my_max_depth(__TBB_INIT_DEPTH) {}
	...
	{  // part of old should_execute_range()
		if (!(self().my_divisor /
			  Mode::my_partition::factor)) {  // if not from the top P tasks of binary tree
			self().my_divisor = 1;	// TODO: replace by on-stack flag (partition_state's member)?
			if (is_stolen_task(ed) &&
				t.my_parent->m_ref_count >= 2) {  // runs concurrently with the left task
			...
			do {
				...
				if (self().check_for_demand(start)) {
					...
					if (range_pool.is_divisible(
							self().max_depth()))  // was not enough depth to fork a task
	...
	bool check_for_demand(Task& t)
	{
		if (pass == my_delay) {
			if (self().my_divisor > 1)	// produce affinitized tasks while they have slot in array
				return true;  // do not do my_max_depth++ here, but be sure range_pool is splittable
							  // once more
			else if (self().my_divisor && my_max_depth) {  // make balancing task
				self().my_divisor =
					0;	// once for each task; depth will be decreased in align_depth()
				return true;
			}
...
public:
	auto_partition_type(const auto_partitioner&) { my_divisor *= __TBB_INITIAL_CHUNKS; }
	...
	void spawn_task(task& t, task_group_context& ctx) { spawn(t, ctx); }
};
...
public:
	...
	void spawn_task(task& t, task_group_context& ctx)
	{
		if (my_divisor) {
			if (!my_array[my_head]) {
				// TODO: consider new ideas with my_array for both affinity and static
				// partitioner's, then code reuse
