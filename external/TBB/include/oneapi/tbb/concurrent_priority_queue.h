...
public:
	...
	explicit concurrent_priority_queue(size_type init_capacity,
									   const allocator_type& alloc = allocator_type())
		: mark(0), my_size(0), my_compare(), data(alloc)
	{
	...
	{
		if (this != &other) {
			// TODO: check if exceptions from std::vector::operator=(vector&&) should be handled
			// separately
			data = std::move(other.data);
			mark = other.mark;
			my_size.store(other.my_size.load(std::memory_order_relaxed), std::memory_order_relaxed);
	...
	/* This operation can be safely used concurrently with other push, try_pop or emplace
	 * operations. */
...
private:
	...
	void handle_operations(cpq_operation* op_list)
	{
		...
		// First pass processes all constant (amortized; reallocation may happen) time pushes and
		// pops.
		while (op_list) {
	...
	{
		__TBB_ASSERT(false,
					 "error: calling tbb::concurrent_priority_queue.push(const value_type&) for "
					 "move-only type");
	}
	...
	/** data has the following structure:
		...
		that have not yet been inserted into the heap, in rawPositions
...
template <typename T, typename Alloc, typename = std::enable_if_t<is_allocator_v<Alloc>>>
