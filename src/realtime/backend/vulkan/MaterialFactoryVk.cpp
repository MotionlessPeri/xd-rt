...
// TODO: refactor the material creation procedure to a more data-oriented form
...
std::shared_ptr<MaterialTemplateVk> MaterialFactoryVk::createDeferredGBufferMaterial(
	std::shared_ptr<VulkanSubpass> subpass) const
{
	// create shaders
	std::vector<std::shared_ptr<VulkanShader>> shaders;
	using namespace std::string_literals;
	shaders.emplace_back(createShader(PROJECT_ROOT + "/backend/vulkan/shader/gbuffer.vert.spv"s,
									  VK_SHADER_STAGE_VERTEX_BIT));
	shaders.emplace_back(createShader(PROJECT_ROOT + "/backend/vulkan/shader/gbuffer.frag.spv"s,
									  VK_SHADER_STAGE_FRAGMENT_BIT));
	...
	}
	{
		...
		std::unordered_map<std::string, uint32_t> nameToBinding{
			{"colorMap", 0}, {"metallicRoughnessMap", 1}, {"normalMap", 2},
			{"aoMap", 3},	 {"emissiveMap", 4},		  {"MaterialParams", 5}};
		layoutNameToIndex.insert({"Material", 1});
		layouts.emplace_back(
			device->createDescriptorSetLayout(layoutDesc, std::move(nameToBinding)));
	}
	...
	// UBONode push constant
	struct UBONode {
		glm::mat4 model;
	};
	pushConstants[0].stageFlags = VK_SHADER_STAGE_VERTEX_BIT;
	pushConstants[0].offset = 0;
	pushConstants[0].size = sizeof(UBONode);
	PipelineLayoutDesc layoutDesc;
	layoutDesc.setLayouts = std::move(layouts);
	layoutDesc.ci.sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO;
	layoutDesc.ci.pNext = nullptr;
	layoutDesc.ci.flags = 0;
	layoutDesc.ci.pushConstantRangeCount = pushConstants.size();
	layoutDesc.ci.pPushConstantRanges = pushConstants.data();
	const auto pipelineLayout = device->createPipelineLayout(std::move(layoutDesc));
	GraphicsPipelineDesc pipelineDesc;
	pipelineDesc.dynamicStates = {VK_DYNAMIC_STATE_VIEWPORT, VK_DYNAMIC_STATE_SCISSOR};
	...
	pipelineDesc.ci.pColorBlendState = &colorBlending;
	pipelineDesc.ci.pDynamicState = &dynamicState;
	// TODO: we might need to use render pass compability here. Getting render
	// pass from some static places but not pass it in.
	// TODO: We might need a RenderPassTraits class for registering render pass
	// compability
	const auto pipeline = subpass->createGraphicsPipeline(std::move(pipelineDesc), pipelineLayout);
	return std::make_shared<MaterialTemplateVk>(device, std::move(shaders),
												std::move(layoutNameToIndex), pipeline);
...
std::shared_ptr<MaterialTemplateVk> MaterialFactoryVk::createDeferredLightMaterial(
	std::shared_ptr<VulkanSubpass> subpass) const
{
	// create shaders
	std::vector<std::shared_ptr<VulkanShader>> shaders;
	using namespace std::string_literals;
	shaders.emplace_back(createShader(PROJECT_ROOT + "/backend/vulkan/shader/lightPass.vert.spv"s,
									  VK_SHADER_STAGE_VERTEX_BIT));
	shaders.emplace_back(createShader(PROJECT_ROOT + "/backend/vulkan/shader/lightPass.frag.spv"s,
									  VK_SHADER_STAGE_FRAGMENT_BIT));
	// declare all desc layouts
	std::unordered_map<std::string, uint32_t> layoutNameToIndex;
	std::vector<std::shared_ptr<VulkanDescriptorSetLayout>> layouts;

	{
		...
		layoutDesc.ci.pBindings = layoutDesc.bindings.data();
		std::unordered_map<std::string, uint32_t> nameToBinding{
			{"inputColor", 0}, {"inputPosition", 0}, {"inputNormal", 0},
			{"inputRMO", 0},   {"inputEmissive", 0},
		};
		layoutNameToIndex.insert({"Inputs", 0});
		layouts.emplace_back(
			device->createDescriptorSetLayout(layoutDesc, std::move(nameToBinding)));
	}
